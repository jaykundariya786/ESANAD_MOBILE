...
import {
  useGetHealthCancelledPolicy,
  useGetMotorCancelledPolicy,
  useGetTravelCancelledPolicy,
...
const CancelledPolicy = () => {
  ...
  const { data: cancelledPolicyData } = useGetMotorCancelledPolicy();
  const { data: cancelledHealthPolicyData } = useGetHealthCancelledPolicy();
  const { data: cancelledTravelPolicyData } = useGetTravelCancelledPolicy();

  const allPolicies = [
    ...(cancelledPolicyData?.filter(
      itm => itm?.response?.Errors?.length === 0,
    ) || []),
    ...(cancelledHealthPolicyData || []),
    // ...(cancelledTravelPolicyData || []),
  ];
  ...
  const ListEmptyComponent = () => (
    <View style={styles.emptyContainer}>
      ...
      />
      <Text style={styles.emptyTitle}>No Cancelled Policies</Text>
      <Text style={styles.emptyDescription}>
        You don't have any cancelled policies at the moment.
      </Text>
    </View>
  );
  ...
  return (
    ...
    >
      <Header title="Cancelled Policies" onBack={navigation.goBack} />
      <FlatList
        data={allPolicies}
