...
const QuoteRow = React.memo(({ item, theme, styles, onRemove }) => {
  const isThirdParty = item?.insuranceType === 'thirdparty';
  const logoUri = `${env.API_URL}${item?.quoteInfo?.company?.logoImg?.path}`;
        ...
        <View style={styles.microTagRail}>
          <View style={styles.microTag}>
            <Text style={styles.microTagText}>
              {isThirdParty ? 'Third Party' : 'Comprehensive'}
            </Text>
          </View>
          <View style={styles.coverTag}>
            <Text style={styles.coverTagText}>
              Cover: AED {formatNumber(item?.quoteInfo?.carValue)}
            </Text>
          </View>
        </View>
      </View>
...
const CompareModal = ({
  ...
  const { mutate: compareQuotes } = useCompareQuotes();
  const insets = useSafeAreaInsets();

  const opacity = useSharedValue(0);
  const [isVisible, setIsVisible] = useState(false);
  const { updateCompareRefID } = useMotorDetalisStore();
  ...
  const containerStyle = useAnimatedStyle(() => ({
    opacity: opacity.value,
    transform: [{ translateY: withTiming(showCompareModal ? 0 : 30) }],
  }));

  const handleCompare = useCallback(() => {
    if (quotesList.length < MIN_PLANS) return;
    ...
    updateCompareRefID(referenceId);
    setShowCompareModal(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [quotesList, referenceId]);
